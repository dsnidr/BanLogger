...
func (s *BanService) CreateBan(ban banlogger.Ban) error {
	query := "INSERT INTO Ban (PlayerID, Duration, Reason, Staff, UnbannedAt, Timestamp) VALUES (?, ?, ?, ?, ?, ?);"

	_, err := s.DB.ExecContext(context.Background(), query,
		ban.PlayerID, ban.Duration, ban.Reason, ban.Staff, ban.UnbannedAt, ban.Timestamp)
...
func (s *BanService) GetBansByPlayerID(playerID string) ([]banlogger.Ban, error) {
	query := "SELECT * FROM Ban WHERE PlayerID = ?;"
	...
	for rows.Next() {
		...
		err = rows.Scan(&ban.ID, &ban.PlayerID, &ban.Duration, &ban.Reason, &ban.Staff, &ban.UnbannedAt, &ban.Timestamp)
		if err != nil {
			return nil, err
		}
...
func (s *BanService) GetCurrentBans() ([]banlogger.Ban, error) {
	query := "SELECT * FROM Ban WHERE UnbannedAt > strftime('%s', 'now') OR UnbannedAt = 0;"
